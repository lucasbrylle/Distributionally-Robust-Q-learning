...
class CliffWalkingEnvironment(gym.Env):
    ...
    def __init__(self, step_down = 0, render_mode: Optional[str] = None):
        self.shape = (8, 12)
    ...
    def A(self, state):
        ...
        if state == 0:
            actions.remove(0)
            actions.remove(3)
        if state % 12 == 0 and state != 0 and state != 84:
            actions.remove(3)
        if state % 12 == 11 and state != 0 and state != 84:
            actions.remove(1)
        if state in list(range(12)) and state != 0 and state != 84:
            actions.remove(0)
        if state > 96:
            actions = []
        return actions
